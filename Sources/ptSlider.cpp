...
void ptSlider::paintEvent(QPaintEvent *)
{
  ...
  optBar.state |= QStyle::State_Horizontal;
...
//    close spinbox without saving value
  ...
  if (m_ValueRect.contains(ev->position().toPoint()))
  {
//  wheel step equals to spinbox step
    setValue(minimum()+(maximum()-minimum())/(m_Maximum.toDouble()-m_Minimum.toDouble())*
             (m_Value.toDouble()+m_Step.toDouble()*ev->angleDelta().y()/120-m_Minimum.toDouble()));
    update();
    ev->accept();
  }
  else
...
void ptSlider::initValueRect()
{
  ...
  int w=55;
  if (m_Type == QVariant::Int)
    w=metrics.horizontalAdvance(m_Maximum.toString()+"     ");
  if (m_Type == QVariant::Double)
    w=metrics.horizontalAdvance(locale().toString(m_Maximum.toDouble(), 'f', m_Decimals)+"     ");
