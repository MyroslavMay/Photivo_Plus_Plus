...
: QMainWindow(NULL)
{
  ...
  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabWhiteBalance"))->
    SetHelpUri("https://photivo.org/manual/tabs/camera#white_balance");

  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabLensfunLensParameters"))->
    SetHelpUri("https://photivo.org/manual/tabs/geometry#lens_parameters");
  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabLensfunCA"))->
    SetHelpUri("https://photivo.org/manual/tabs/geometry#chromatic_aberration");
  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabLensfunVignette"))->
    SetHelpUri("https://photivo.org/manual/tabs/geometry#vignetting");
  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabLensfunDistortion"))->
    SetHelpUri("https://photivo.org/manual/tabs/geometry#lens_distortion");
  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabLensfunGeometry"))->
    SetHelpUri("https://photivo.org/manual/tabs/geometry#geometry_conversion");
  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabDefish"))->
    SetHelpUri("https://photivo.org/manual/tabs/geometry#defish");
  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabRotation"))->
    SetHelpUri("https://photivo.org/manual/tabs/geometry#rotation_and_perspective");
  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabCrop"))->
    SetHelpUri("https://photivo.org/manual/tabs/geometry#crop");
  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabLiquidRescale"))->
    SetHelpUri("https://photivo.org/manual/tabs/geometry#seam_carving");
  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabResize"))->
    SetHelpUri("https://photivo.org/manual/tabs/geometry#resize");
  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabFlip"))->
    SetHelpUri("https://photivo.org/manual/tabs/geometry#flip");
  dynamic_cast<ptGroupBox*>(m_GroupBox->value("TabBlock"))->
    SetHelpUri("https://photivo.org/manual/tabs/geometry#block");
  ...
  for (short itTab=0; itTab<ToolBoxes.size(); itTab++) {
    ...
    for (short itIdx=0; itIdx<ToolBoxes[itTab]->count(); itIdx++) {
      ...
      GInfo->Assert(DirectChildrenOfPage.size() == 1,
                    ...
                    "corresponding new-style filter in ptMain::CreateAllFilters().", PT_SRC_LOCATION);
...
void ptMainWindow::OnToolBoxesEnabledTriggered(const bool Enabled) {
  ...
  if (Enabled) {
    GInfo->Raise("Toolbox mode is ancient and dysfunctional!", PT_SRC_LOCATION);
  }
}
...
void ptMainWindow::keyPressEvent(QKeyEvent *Event) {
  ...
  if (Settings->GetInt("BlockTools")==0) {
    if (Event->key()==Qt::Key_F1) {
      QDesktopServices::openUrl(QString("https://photivo.org/manual"));
      ...
      const QRect availableGeom = this->screen()->availableGeometry();
      move(QPoint(
        qMax((availableGeom.width() - 1200), 0) / 2,
        qMax((availableGeom.height() - 900) / 2 - 20, 0)
      ));
...
void ptMainWindow::wheelEvent(QWheelEvent * Event) {
  if (Event->angleDelta().y() < 0 && ((QMouseEvent*)Event)->modifiers()==Qt::AltModifier) {
    ProcessingTabBook->setCurrentIndex(MIN(ProcessingTabBook->currentIndex()+1,ProcessingTabBook->count()));
  } else if (Event->angleDelta().y() > 0 && ((QMouseEvent*)Event)->modifiers()==Qt::AltModifier) {
    ProcessingTabBook->setCurrentIndex(MAX(ProcessingTabBook->currentIndex()-1,0));
  }
}
...
void ptMainWindow::SwitchUIState(const ptUIState AState)
{
  ...
  switch (AState) {
    ...
    case uisBatch:
      ...
      break;
    default:
      GInfo->Raise("Unknown UI state", PT_SRC_LOCATION);
  }
}


QStandardItem* ptMainWindow::visibleToolsItemForFilter(const ptTempFilterBase& filter)
{
    // first level in the visible tools model corresponds to the processing tabs
    Q_ASSERT(ProcessingTabBook->count() == m_VisibleToolsModel->rowCount());

    // find all TreeItems with required name (in different tabs)
    const QList<QStandardItem *> itemList =
        m_VisibleToolsModel->findItems(filter.caption(), Qt::MatchExactly | Qt::MatchRecursive);
    Q_ASSERT(not itemList.isEmpty());

    // Find item in the correct proper tab.
    // Tab order in the processing view and the visible tools model is the same. We can compare
    // the numerical tab indexes to find the correct one.
    QStandardItem *item = nullptr;

    for (const auto& candidate : itemList) {
      const int vtTabIdx = m_VisibleToolsModel->indexFromItem(candidate).parent().row();

      if (vtTabIdx == filter.parentTabIdx()) {
          item = candidate;
          break;
      }
    }
...
void ptMainWindow::ApplyVisibleTools() {
  ...
  foreach (QString hBoxName, *m_GroupBoxesOrdered) {
    auto filter = m_GroupBox->value(hBoxName);
    Q_ASSERT(filter);
    QStandardItem *item = visibleToolsItemForFilter(*filter);
    ...
    if (item->data(Qt::UserRole+1).toInt() == tsHidden) {
      ...
      if (!hiddenTools.contains(hBoxName)) {
        ...
        filter->guiWidget()->hide();
        // find first active tool, which changes it's state
        if (filter->isActive() && FirstActive.size() == 0)
...
void ptMainWindow::UpdateVisibleTools() {
  const QStringList hiddenTools = Settings->GetStringList("HiddenTools");
  const QStringList favouriteTools = Settings->GetStringList("FavouriteTools");

  // iterate over all existing group boxes
  for (const QString& groupBoxName : *m_GroupBoxesOrdered) {
    auto filter = m_GroupBox->value(groupBoxName);
    Q_ASSERT(filter);
    QStandardItem *item = visibleToolsItemForFilter(*filter);

    // set tool state and corresponding icon will be set automatically
    if (hiddenTools.contains(groupBoxName)) {
      m_VisibleToolsModel->setData(m_VisibleToolsModel->indexFromItem(item),
                                   tsHidden, Qt::UserRole+1);
    } else {
      if (favouriteTools.contains(groupBoxName)) {
        m_VisibleToolsModel->setData(m_VisibleToolsModel->indexFromItem(item),
    ...
    // check if tool can be hidden and set corresponding flag
    if (Settings->ToolAlwaysVisible(groupBoxName)) {
