...
void ptCurveWindow::setValue(const QVariant &AValue) {
  GInfo->Assert(AValue.type() == QVariant::Map,
                QString("%1: Value must be of type QVariant::Map (8), but is (%2).")
                    .arg(this->objectName()).arg(AValue.type()), PT_SRC_LOCATION);
...
void ptCurveWindow::execContextMenu(const QPoint APos) {
  ...
  default:
    GInfo->Raise(QString("Unhandled curve interpolation type: ") + QString::number((int)FCurve->interpolType()), PT_SRC_LOCATION);
  }
  ...
  if (FCurve->supportedMasks() == (ptCurve::LumaMask | ptCurve::ChromaMask)) {
    ...
    default:
      GInfo->Raise(QString("Unhandled curve mask type: ") + QString::number((int)FCurve->mask()), PT_SRC_LOCATION);
    }
  }
