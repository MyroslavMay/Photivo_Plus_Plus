...
//              UserDirectory + "photivoLogo.png");
  ...
  QStringList UiLanguages = TranslDir.entryList(
      QStringList("photivo_*.qm"),
  ...
  if (TranslMode == 1) {
      ...
      if (LangIdx >= 0) {
          ...
          auto ok1 = appTranslator.load("photivo_" + UiLanguages[LangIdx], UserDirectory + "Translations");
          Q_UNUSED(ok1);
          TheApplication->installTranslator(&appTranslator);
          auto ok2 = qtTranslator.load("qt_" + UiLanguages[LangIdx], UserDirectory + "Translations");
          Q_UNUSED(ok2);
  ...
  MainWindow->restoreGeometry(Settings->m_IniSettings->value("MainWindowGeometry").toByteArray());
  ...
  try {
    ...
    GInfo->Raise(E.what(), PT_SRC_LOCATION);
  } catch (...) {
    GInfo->Raise("Unknown error.", PT_SRC_LOCATION);
  }
  return 1;
}
...
void PrepareTags(const QString TagsInput) {
  ...
  QStringList DigikamTags = WorkString.split(",", Qt::SkipEmptyParts);
...
void CB_MenuFileExit(const short) {
  ...
  Settings->m_IniSettings->setValue("MainWindowGeometry", MainWindow->saveGeometry());
  Settings->m_IniSettings->setValue("IsMaximized",
                                    static_cast<bool>(MainWindow->windowState() & Qt::WindowMaximized));
...
void CB_OutputColorProfileResetButton() {
  Settings->SetValue(
      "OutputColorProfile",
      Settings->GetString("UserDirectory") + "Profiles/Output/sRGB.icc");
...
void SetBackgroundColor(int SetIt) {
  if (SetIt) {
    QPalette BGPal;
    BGPal.setColor(QPalette::Window, QColor(Settings->GetInt("BackgroundRed"),
...
void ReadSidecar(const QString& Sidecar)
{
  ...
  auto hImage = Exiv2::ImageFactory::open(Sidecar.toLocal8Bit().data());
