...
void ptInput::init(const ptCfgItem &ACfgItem) {
  ...
  GInfo->Assert(!m_SpinBox, "Trying to initialize an already initalized input widget.", PT_SRC_LOCATION);
  if (!(ACfgItem.Type == ptCfgItem::SpinEdit || ACfgItem.Type == ptCfgItem::Slider ||
        ACfgItem.Type == ptCfgItem::HueSlider))
  {
    GInfo->Raise(QString("Wrong GUI type (%1). Must be a spin edit or slider.").arg(ACfgItem.Type), PT_SRC_LOCATION);
  }
