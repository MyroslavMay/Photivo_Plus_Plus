...
/*
The Lensfun API changed quite a bit over the versions. We distinguish 3 eras:
+ the big API overhaul introduced in 0.3.95
+ the older 0.3.x API
+ the 0.2.x API (very similar to the older 0.3.x API)
*/
#define PT_LENSFUN_VER_0_3_95 ((3 << 16) | (95 << 8))
#define PT_LENSFUN_VER_0_3_0  (3 << 16)
                      ...
                      short ProcessorMode)
{
    ...
    switch(Phase) {
      ...
      case ptProcessorPhase_Raw:
        ...
        if (!Settings->useRAWHandling()) {
          ...
          TRACEMAIN("Opened bitmap at %d ms.", runTimerElapsed());

        } else { // we have a RAW image!

          TRACEMAIN("Starting DcRaw at %d ms.",runTimerElapsed());
          switch (SubPhase) {
            ...
            case ptProcessorPhase_Highlights: {
              ...
              TRACEMAIN("Done Highlights at %d ms.", runTimerElapsed());
              ...
              case ptCameraColor::Profile: {
                ...
                if (!(PathInfo.exists() && PathInfo.isFile() &&
                  ...
                  TRACEMAIN("Not found profile at %d ms.", runTimerElapsed());
            ...
            default: // Should not happen.
              GInfo->Raise(QString("Processor subphase ") + QString::number(SubPhase) + QString(" does not exist."), PT_SRC_LOCATION);
          }
        }
      ...
      case ptProcessorPhase_EyeCandy : // Run EyeCandy.
        ...
        if (m_Image_AfterEyeCandy->m_ColorSpace == ptSpace_Lab) {
          ...
          TRACEMAIN("Done conversion to RGB at %d ms.",runTimerElapsed());
        }
        ...
        if (hFilter->isActive()) {
          ...
          hFilter->runFilter(m_Image_AfterEyeCandy);
          TRACEMAIN("Done R Curve at %d ms.",runTimerElapsed());
        }
        ...
        if (hFilter->isActive()) {
          ...
          hFilter->runFilter(m_Image_AfterEyeCandy);
          TRACEMAIN("Done G Curve at %d ms.",runTimerElapsed());
        }
        ...
        if (hFilter->isActive()) {
          ...
          hFilter->runFilter(m_Image_AfterEyeCandy);
          TRACEMAIN("Done B Curve at %d ms.",runTimerElapsed());
        }
  ...
  Exit:
      ...
      case ptProcessorPhase_Output : // Run Output.
        ...
        TRACEMAIN("Done pipe processing at %d ms.",runTimerElapsed());
...
void ptProcessor::RunLocalEdit(ptProcessorStopBefore StopBefore) {
  ...
  if (!Settings->useRAWHandling()) {
    ...
    if (StopBefore == ptProcessorStopBefore::NoStop) {
      ...
      TRACEMAIN("Done bitmap transfer at %d ms.", runTimerElapsed());
    }
...
void ptProcessor::RunGeometry(ptProcessorStopBefore StopBefore) {
      ...
      Settings->ToolIsActive("TabLensfunDistortion") || Settings->ToolIsActive("TabLensfunGeometry") )
  {
    ...
    switch (VignetteData.Model) {
      ...
      case LF_VIGNETTING_MODEL_PA:
        wantedModflags |= LF_MODIFY_VIGNETTING;
    ...
    switch (DistortionData.Model) {
      ...
      case LF_DIST_MODEL_POLY3:
        wantedModflags |= LF_MODIFY_DISTORTION;
      ...
      case LF_DIST_MODEL_POLY5:
        ...
        DistortionData.Terms[2] = 0.0;
        break;
#if LF_VERSION < PT_LENSFUN_VER_0_3_0
    ...
    constexpr lfPixelFormat pixelFormat = LF_PF_U16;  // current image is uint16 data
    constexpr float cropFactor = 1.0f;  // always already normalised to 35mm equiv

    float scaleFactor = 0.0f;   // 0.0 is lensfun's magic value for auto scaling
    if (Settings->GetInt("LfunAutoScale") == 0) {
      scaleFactor = Settings->GetDouble("LfunScale");
    }
    if (qAbs(scaleFactor - 1.0f) > 0.001f) {
      wantedModflags |= LF_MODIFY_SCALE;
    }

    const float imgFocalLength = Settings->GetDouble("LfunFocal");
    const float aperture = Settings->GetDouble("LfunAperture");
    const float distance = Settings->GetDouble("LfunDistance");

    lfLens lensData{};
    lensData.Type = static_cast<lfLensType>(Settings->GetInt("LfunSrcGeo"));
    ...
    const auto targetGeometry = static_cast<lfLensType>(Settings->GetInt("LfunTargetGeo"));
    if (lensData.Type != targetGeometry) {
      wantedModflags |= LF_MODIFY_GEOMETRY;
    }

#if LF_VERSION < PT_LENSFUN_VER_0_3_95  // using pre-overhaul Lensfun API
#if LF_VERSION >= PT_LENSFUN_VER_0_3_0  // but not the even older 0.2.x API
    lensData.CropFactor = cropFactor;
    lensData.AspectRatio = 1.0f;
#endif

    lensData.GuessParameters();
    assert(lensData.Check());

    lfModifier* modifier = lfModifier::Create(
        &lensData, cropFactor, m_Image_AfterGeometry->m_Width, m_Image_AfterGeometry->m_Height);

    // Init modifier and get list of lensfun actions that actually get performed
    const auto actualModflags = modifier->Initialize(
        &lensData,
        pixelFormat,
        imgFocalLength,
        aperture,
    ...
    m_Image_AfterGeometry->Lensfun(actualModflags, modifier);

    modifier->Destroy();

#else  // using new Lensfun API
    assert(lensData.Check());
    ...
    int actualModflags = 0;

    if (wantedModflags & LF_MODIFY_TCA) {
      actualModflags |= modifier.EnableTCACorrection(&lensData, imgFocalLength);
    }

    if (wantedModflags & LF_MODIFY_VIGNETTING) {
      actualModflags |= modifier.EnableVignettingCorrection(&lensData, imgFocalLength, aperture, distance);
    }

    if (wantedModflags & LF_MODIFY_DISTORTION) {
      actualModflags |= modifier.EnableDistortionCorrection(&lensData, imgFocalLength);
    }

    if (wantedModflags & LF_MODIFY_SCALE) {
      actualModflags |= modifier.EnableScaling(scaleFactor);
    }

    if (wantedModflags &LF_MODIFY_GEOMETRY) {
      actualModflags |= modifier.EnableProjectionTransform(&lensData, imgFocalLength, targetGeometry);
    }
    ...
    if (StopBefore == ptProcessorStopBefore::NoStop) {
      TRACEMAIN("Done Lensfun corrections at %d ms.",runTimerElapsed());
    }
  }
  ...
  if (Settings->ToolIsActive("TabDefish"))
  {
    ...
    constexpr lfPixelFormat pixelFormat = LF_PF_U16;  // current image is uint16 data
    constexpr float cropFactor = 1.0f;  // always already normalised to 35mm equiv
    constexpr lfLensType targetGeometry = LF_RECTILINEAR;
    const float imgFocalLength = Settings->GetDouble("DefishFocalLength");

    float scaleFactor = 0.0f;   // 0.0 is lensfun's magic value for auto scaling
    if (Settings->GetInt("DefishAutoScale") == 0) {
      scaleFactor = Settings->GetDouble("DefishScale");
    }
    const bool isScaling = qAbs(scaleFactor - 1.0f) > 0.001f;

    lfLens lensData{};
    lensData.Type = LF_FISHEYE;
    lensData.SetMaker("Photivo Custom");
    lensData.SetModel("Photivo Custom");
    lensData.AddMount("Photivo Custom");
    ...
    // Init modifier and get list of lensfun actions that actually get performed
    modflags = modifier->Initialize(
        &lensData,
        pixelFormat,
        imgFocalLength,
        1.0f, // doesn't matter for defish
        1.0f, // doesn't matter for defish
        scaleFactor,
        targetGeometry,
        modflags,
    ...
    // Execute lensfun corrections. For vignetting the image is changed in place.
    // For everything else new pixel coordinates are returned in TransformedCoords.
    m_Image_AfterGeometry->Lensfun(modflags, modifier);
    ...
    int modflags = modifier.EnableProjectionTransform(&lensData, imgFocalLength, targetGeometry);

    if (isScaling) {
      modflags |= modifier.EnableScaling(scaleFactor);
    }
    ...
    if (StopBefore == ptProcessorStopBefore::NoStop) {
      TRACEMAIN("Done defish correction at %d ms.",runTimerElapsed());
    }
  }
  ...
  if (Settings->ToolIsActive("TabRotation")) {
    ...
    if (StopBefore == ptProcessorStopBefore::NoStop) {
      TRACEMAIN("Done perspective at %d ms.",runTimerElapsed());
    }
  }
  ...
  if (Settings->ToolIsActive("TabCrop")) {
    ...
    if (((Settings->GetInt("CropX") >> TmpScaled) + (Settings->GetInt("CropW") >> TmpScaled))
      ...
      if (StopBefore == ptProcessorStopBefore::NoStop) {
        TRACEMAIN("Done cropping at %d ms.",runTimerElapsed());
      }
    }
  }
  ...
  if (Settings->ToolIsActive("TabLiquidRescale")) {
    ...
    if (StopBefore == ptProcessorStopBefore::NoStop) {
      TRACEMAIN("Done seam carving at %d ms.",runTimerElapsed());
    }
  }
  ...
  if (Settings->ToolIsActive("TabResize")) {
    ...
    if (StopBefore == ptProcessorStopBefore::NoStop) {
      TRACEMAIN("Done resize at %d ms.",runTimerElapsed());
    }
  }
  ...
  if (Settings->ToolIsActive("TabFlip")) {
    ...
    if (StopBefore == ptProcessorStopBefore::NoStop) {
      TRACEMAIN("Done flip at %d ms.",runTimerElapsed());
    }
  }
