...
void ptChoice::init(const ptCfgItem &ACfgItem) {
  GInfo->Assert(this->parent(), "Parent cannot be null.", PT_SRC_LOCATION);
  GInfo->Assert(!m_ComboBox, "Trying to initialize an already initalized check box.", PT_SRC_LOCATION);
  GInfo->Assert(ACfgItem.Type == ptCfgItem::Combo,
                QString("Wrong GUI type (%1). Must be ptCfgItem::Combo.").arg(ACfgItem.Type), PT_SRC_LOCATION);
...
void ptChoice::setValue(const QVariant &AValue) {
  ...
  if (idx == -1) {
    GInfo->Raise("Invalid combo value: " + AValue.toString(), PT_SRC_LOCATION);
  } else {
