...
void ptToolBox::createGui() {
  GInfo->Assert(FBodyWidget, "The toolbox cannot have a null body widget.", PT_SRC_LOCATION);
  GInfo->Assert(FFilter, "The filter associated with the toolbox cannot be null.", PT_SRC_LOCATION);
  ...
  GInfo->Assert(hBodyLayout, QString("Error! GUI widget of filter \"%1\" does not have a layout.")
                             .arg(FFilter->uniqueName()), PT_SRC_LOCATION);
