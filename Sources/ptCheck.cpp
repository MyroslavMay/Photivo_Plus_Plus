...
void ptCheck::init(const ptCfgItem &ACfgItem) {
  ...
  GInfo->Assert(!m_CheckBox, "Trying to initialize an already initalized check box.", PT_SRC_LOCATION);
  GInfo->Assert(ACfgItem.Type == ptCfgItem::Check,
                QString("Wrong GUI type (%1). Must be ptCfgItem::Check.").arg(ACfgItem.Type), PT_SRC_LOCATION);
