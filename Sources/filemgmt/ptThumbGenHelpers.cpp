...
TThumbAssoc ptThumbQueue::dequeue() {
  ...
  if (FItems.empty()) {
    ...
    TThumbAssoc result = FItems.front();
    FItems.pop_front();
    return result;
  }
}
...
void ptThumbQueue::enqueue(const TThumbAssoc& AItem, TThumbQPrio APriority) {
  ptMutexLocker hLock(&FItemsMutex);
  switch (APriority) {
    case TThumbQPrio::High: FItems.push_front(AItem); break;
    case TThumbQPrio::Normal: FItems.push_back(AItem); break;
    default: GInfo->Raise("Unexpected case branch", PT_SRC_LOCATION);
  }
}
...
void ptThumbQueue::enqueue(const QList<TThumbAssoc>& AItems, TThumbQPrio APriority) {
  ...
  for (auto& hItem: AItems) {
    switch (APriority) {
      case TThumbQPrio::High: FItems.push_front(hItem); break;
      case TThumbQPrio::Normal: FItems.push_back(hItem); break;
      default: GInfo->Raise("Unexpected case branch", PT_SRC_LOCATION);
    }
  }
}
