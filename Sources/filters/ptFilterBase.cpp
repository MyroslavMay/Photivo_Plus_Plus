...
QString ptFilterBase::uniqueName() const {
  GInfo->Assert(!FUniqueName.isEmpty(),
                QString("No unique name set for filter \"%1\".") .arg(FFilterName), PT_SRC_LOCATION);
...
void ptFilterBase::init(const QString &AUniqueName, const QString &AGuiNamePostfix) {
  GInfo->Assert(!AUniqueName.isEmpty(),
                QString("Unique name for filter \"%1\" cannot be empty.").arg(FFilterName), PT_SRC_LOCATION);
...
ptWidget *ptFilterBase::findPtWidget(const QString &AId, QWidget *AWidget) {
  ...
  if (!hWidget) {
    GInfo->Raise(QString("Widget \"%1\" not found.").arg(AId),
                 QString(FFilterName+"/"+FUniqueName+": "+PT_SRC_LOCATION).toLocal8Bit().data());
  }
...
void ptFilterBase::connectCommonDispatch() {
  GInfo->Assert(!FGuiContainer, "The filter's ("+FFilterName+") GUI must be created first.", PT_SRC_LOCATION);
...
ptWidget* ptFilterBase::createWidgetByType(const ptCfgItem &ACfgItem, QWidget *AParent) {
  ...
  default:
    GInfo->Raise(QString("Unhandled ptCfgItem::TType value: %1.").arg(ACfgItem.Type), PT_SRC_LOCATION);
    break;
  }
...
void ptFilterBase::updateGui(const bool ARequestPipeRun /*= true*/) {
  if (FGuiContainer) {
    ...
    for(const ptCfgItem &hCfgItem: FConfig.items()) {
      ...
      if (!hWidget) {
        GInfo->Warning(QString("%1: Widget \"%2\" not found in GUI.").arg(uniqueName(), hCfgItem.Id), PT_SRC_LOCATION);
        continue;
      }
...
void ptFilterBase::createGui() {
  GInfo->Assert(!FGuiContainer, "GUI already created.", PT_SRC_LOCATION);
  GInfo->Assert(!FConfig.isEmpty(), "FConfig object must be initialized first.", PT_SRC_LOCATION);
