...
bool ptImageHelper::WriteExif(const QString              &AFileName,
  ...
  try {
    ...
    if (AExifBuffer.data() != NULL && AExifBuffer.size() > 0) {
    ...
    auto exiv2Image = Exiv2::ImageFactory::open(AFileName.toLocal8Bit().data());
                             ...
                             std::vector<uint8_t> &AExifBuffer)
{
  ...
  try {
    ...
    auto hImage = Exiv2::ImageFactory::open(AFileName.toLocal8Bit().data());
...
bool ptImageHelper::TransferExif(const QString ASourceFile, const QString ATargetFile)
{
  ...
  try {
    ...
    auto hSourceImage = Exiv2::ImageFactory::open(ASourceFile.toLocal8Bit().data());

    if (!hSourceImage.get()) return false;

    hSourceImage->readMetadata();

    auto hTargetImage = Exiv2::ImageFactory::open(ATargetFile.toLocal8Bit().data());
