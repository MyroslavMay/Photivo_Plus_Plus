...
void ptCurve::calcCurve() {
  ...
  default:
    GInfo->Raise(QString("Unhandled interpolation type: ") + QString::number(FInterpolType), PT_SRC_LOCATION);
  }
}
...
void ptCurve::calcSplineCurve() {
  ...
  GInfo->Assert(ypp, "spline_cubic_set() returned a nullptr.", PT_SRC_LOCATION);
...
int ptCurve::readCurveFile(const QString &AFileName, const bool AOnlyAnchors) {
  ...
  int hNextIdx = hCurveData.indexOf(QRegularExpression("^CurveType  *[0-1]$"));
                                   ...
                                   double ybcbeg, int ibcend, double ybcend )
{
  ...
  for (; i < n - 1; i++ ) {
    if ( t[i+1] <= t[i] ) {
      GInfo->Raise(QString("The knots must be strictly increasing, but T(%1) = %2, T(%3) = %4")
                     .arg(i).arg(t[i]).arg(i+1).arg(t[i+1]), PT_SRC_LOCATION);
    }
  }
  ...
  if ( ibcbeg == 0 ) {
    ...
    FREE(a);
    FREE(b);
    GInfo->Raise(QString("IBCBEG must be 0, 1 or 2, but the input value is %1.").arg(ibcbeg), PT_SRC_LOCATION);
  }
  //
  ...
  if ( ibcend == 0 ) {
    ...
    FREE(a);
    FREE(b);
    GInfo->Raise(QString("IBCEND must be 0, 1 or 2, but the input value is %1.").arg(ibcend), PT_SRC_LOCATION);
  }
  //
  ...
  if ( n == 2 && ibcbeg == 0 && ibcend == 0 ) {
    ...
    if ( !ypp ) {
      FREE(a);
      FREE(b);
      GInfo->Raise("The linear system could not be solved.", PT_SRC_LOCATION);
    }
  }
